...
export class Storage {
    ...
    constructor(repository: string, production: boolean = (process.env.production == 'true')) {
        this.backend = production ? new IpfsStorage(repository) : new DummyStorage(repository);
        this.encryption = production ? new WebCrypto() : new DummyEncryption();
    }

    public async store(order: Order): Promise<string> {
        return this.backend.store(this.encode(order));
    }
    public async load(id: string): Promise<Order> {
        return this.decode(await this.backend.load(id));
     }
    public async list(): Promise<Order[]> {
        return (await this.backend.list()).map(this.decode)
     }

    private encoder = new TextEncoder();
    private decoder = new TextDecoder();

    private encode(order: Order): Uint8Array {
        return this.encoder.encode(JSON.stringify(order));
    }

    private decode(data: Uint8Array): Order {
